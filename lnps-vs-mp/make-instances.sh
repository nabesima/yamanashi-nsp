        ...
        for STAFF_REQUEST in "${STAFF_REQUEST_LIST[@]}"; do
            if [ "$NURSES" -ge 20 ]; then
                for PAIR in "${PAIRS[@]}"; do
                    ...
                    echo "Generating NSP instance: Nurses=${NURSES}, Days=${DAYS}, Staff Request=${STAFF_REQUEST}, Recommended Pairs=${RECOMMENDED_PAIRS}, Forbidden Pairs=${FORBIDDEN_PAIRS}, Schedule Offset=${SCHEDULE_OFFSET}, Seed=${SEED}, No priority"
                    ../gennsp.py -n "$NURSES" -d "$DAYS" -s "$START_DATE" -r "$STAFF_REQUEST" -rp "$RECOMMENDED_PAIRS" -fp "$FORBIDDEN_PAIRS" --schedule-offset "$SCHEDULE_OFFSET" --seed "$SEED" --no-priority > "$OUTPUT_FILE"
                done
            else
                RECOMMENDED_PAIRS=0
                FORBIDDEN_PAIRS=0
